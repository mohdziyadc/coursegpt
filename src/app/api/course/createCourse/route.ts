...
export async function POST(req: Request, res: Response) {
  try {
    ...
    const { title, units } = createCourseSchema.parse(body); // destructuring title and units by parsing the body with our schema
    console.log(`Title: ${title}\nUnits: ${units}`);
    // Each unit would contain a unit title and an array of chapters
    type outputUnit = {
      title: string;
      chapters: {
    ...
    // To make sure that the response is of JSON format
    const outputUnits: outputUnit = await strict_output(
    ...
    // Decrement the credits if a course is created
